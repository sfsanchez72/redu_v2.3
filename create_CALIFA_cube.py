...
def deconv_img(img_center,aper_center,FWHM_input=1,slope=0.75,smooth=0,clip=False,filter_epsilon=0.005):
...
def deconv_img_psf(img_center,img_psf,FWHM_input=1,smooth=0,clip=False,filter_epsilon=0.005):
    sigma_FWHM=FWHM_input/pix_size/2.354
    (ny,nx)=img_center.shape
    img_loop_in = img_center+img_center.max() * 1E-5 * np.random.random(img_center.shape)
    img_loop_dec = restoration.richardson_lucy(img_loop_in, img_psf_r,10,                                               clip=clip,filter_epsilon=filter_epsilon)#, num_iter=30)
...
#            rat_mean=weighted_median(rat_data[mask_data],1/(1+map_dist_CAL[mask_data]))
            ...
            if ((np.isnan(rat_mean)) or (rat_mean<0)):
                rat_mean=np.nanmedian(rat_data[mask_data & (rat_data>0)])
...
if (nargs>=2):
    name=sys.argv[1]
    if (nargs>=3):
        master_flat=int(sys.argv[2])
        if (nargs>=4):
            f_e=float(sys.argv[3])
            if (nargs>=5):
                fsize=float(sys.argv[4])
                if (nargs>=6):
                    master_fs=float(sys.argv[5])
                    if (nargs>=7):
                        over_reg=int(sys.argv[6])
                        if (nargs>=8):
                            slope=float(sys.argv[7])
                            if (nargs>=9):
                                slope_var=float(sys.argv[8])
else:
    print(f'USE: create_CALIFA_cube.py NAME [master_flat={master_flat}] [f_e={f_e}] [fsize={fsize}] [master_flux_scale={master_fs}] [override_registration={over_reg}] [slope={slope}] [slope_var={slope_var}]')
    print('Run from:')
    print('/disk-c/sanchez/CALIFA/ancillary')
    exit()

    
#print(f'{name},{master_flat}')
...
file_mf = f'{fdir_out}/master_flat.fits.gz'

hdu_mf = fits.open(file_mf)
img_mf = hdu_mf[0].data
(ny_mf,nx_mf)=img_mf.shape
...
rss_err[rss_err>(med_err+3*std_err)]=med_err+2*std_err


# In[13]:


#primhdu = fits.PrimaryHDU(data=rss_err)
#hdulist=fits.HDUList([primhdu])
#outfile=f'out/{name}.rss_err_new.fits.gz'
#hdulist.writeto(outfile,overwrite=True)
#print(f'med_error ={med_err} +- {std_err}')
#quit()
...
#    dist_vig_now = dist_vig[mask_vig]
#    print(len(ind_spec_vig))
    for i_vig in arange(len(ind_spec_vig)):
        n_spec_vig=ind_spec_vig[i_vig]
        (ny_rss,nx_rss)=rss_data.shape
        nx_rss_0 = int(0.45*nx_rss)
        nx_rss_1 = int(0.55*nx_rss)
        spec_now = rss_data[n_spec_vig,:]
        spec_now_cen= spec_now[nx_rss_0:nx_rss_1]
        med=np.nanmedian(spec_now_cen[spec_now_cen!=0])
        ...
        for j in arange(0,nk):
            ...
            med_j=np.nanmedian(rss_data[n_spec_nvig,nx_rss_0:nx_rss_1][spec_now_cen!=0])  
...
#            weights.append(1/chi_sq)
            weights.append(1/dist_vig[i_vig][j])
            rss_near[j,:]=spec_scaled
        weights=np.array(weights)
        spec_near=np.nanmedian(rss_near,axis=0)#average(rss_near,axis=0,weights=weights)
...
(nz_org,ny_org,nx_org)=cube_org.shape
flat_mf = np.ones((ny_org,nx_org))
if (nx_mf>nx_org):
    nx_mf=nx_org
if (ny_mf>ny_org):
    ny_mf=ny_org
flat_mf[0:ny_mf,0:nx_mf]=img_mf[0:ny_mf,0:nx_mf]
...
try:
    ...
    dar_flag=1
    if (len(wavelength)!=len(spec2_x._wave)):
        (spec2_x,spec2_y)=measureDARPeak_py3d(file_cube_org, out_prefix, coadd=20, smooth_poly=1, start_wave='3900', end_wave='7100',out_vals='1',figure_out=out_DAR_file, bary_box=2)
        dar_flag=2
except:
    dar_flag=0
    print('*** No DAR determined\n')
    (nz_o,ny_o,nx_o)=cube_org.shape
    spec2_y =Spectrum1D(data=(nx_o/2)+0.0*wavelength, wave=wavelength)
    spec2_x =Spectrum1D(data=(ny_o/2)+0.0*wavelength, wave=wavelength)
    

if (len(wavelength)!=len(spec2_x._wave)):
    spec2_x._wave=wavelength
    spec2_y._wave=wavelength
    (nz_o,ny_o,nx_o)=cube_org.shape
    spec2_x._data=(nx_o/2)+0.0*wavelength
    spec2_y._data=(ny_o/2)+0.0*wavelength
...
img_dec_r = deconv_img(img_r,aper,FWHM_input=0.5,slope=slope,smooth=1,clip=False,filter_epsilon=0.00005)
...
for iz in arange(nz_0,nz_1):
    ...
    for i_mc in arange(0,n_mc):
        ...
        img_loop_dec=deconv_img_psf(img_org,img_psf_r,FWHM_input=1,smooth=0,clip=False,filter_epsilon=f_e)       
...
#    cube_dec[iz,:,:]=np.nanmean(cube_mc,axis=0)
    cube_dec[iz,:,:]=np.nanmedian(cube_mc,axis=0)
...
if (dar_flag==1):
    try:
        ...
        if (len(wavelength)!=len(spec2_x._wave)):
            (spec2_x,spec2_y)=measureDARPeak_data(cube_py3d, out_prefix, coadd=20, smooth_poly=1, start_wave='3900', end_wave='7100',out_vals='1',figure_out=out_DAR_file,bary_box=2)
    except:
        print('*** No DAR determined, deconvolved cube\n')
...
rat_map=rat_map_r
flux_scale=1.0
if ((rat_mean_g>0)&(rat_mean_r>0)):
    flux_scale = 0.5*(rat_mean_g+rat_mean_r)
else:
    if ((rat_mean_g>0)&(rat_mean_r<=0)):
        flux_scale = rat_mean_g
    else:
        if ((rat_mean_r>0)&(rat_mean_g<=0)):
            flux_scale = rat_mean_r
if (flux_scale>1e2):
    flux_scale=1
...
# Using master flat for regions with low-SN
#rat_map_inv[mask_good_pix]=1

scale_mf=1.3
scale_mf = np.nanmean(flat_mf[~mask_good_pix]/rat_map_inv[~mask_good_pix])

rat_map_inv[mask_good_pix]=flat_mf[mask_good_pix]/scale_mf
...
if (master_flat==0):
    ...
    print('*** Using the master flat_mf ***')
    cube_dec_flat=cube_dec*flat_mf/master_fs
...
try:
    ...
    gaia_mask_org=get_mask_gaia(primhdu,name,CATALOG_PKL='../Artemi/gaia_dr3_sn5.pkl',psf=2.5,mag_lim=None,plot=0)  
