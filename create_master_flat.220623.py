...
list_obj=['2','NGC0171','IC0208','IC0522','IC4566','iPTF13ebh',\
          'LSBCF560-04','M64NW','M64SE','M82','NGC0036','NGC0155']

for file in sorted(os.listdir(out_dir)):
    if file.endswith(".V500.drscube.fits.gz"):
        name=file
        name = name.replace(".V500.drscube.fits.gz",".r.fits.gz")
        if (name in list_obj):
            nz=nz+1;
    ...
    if file.endswith(".V500.drscube.fits.gz"):
        if (iz<nz):
            ...
            try:
                ...
                flux_scale=np.nan
            flux_now=flux_scale
            if ((flux_scale>4)|(flux_scale<0.2)):
                flux_now=np.nan
...
flat_cube[(flat_cube>3)|(flat_cube<0.3)|(r_cube>1)]=np.nan
