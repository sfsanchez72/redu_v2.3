...
#    plt.show()
    ...
    norm = matplotlib.colors.LogNorm(vmin=10.0/float(np.abs(best_valid)),vmax=numpy.max(chisq))
...
i_range=5
filename_PS = f'{name}_PS_warp_{band}.fits'
qc_file=f'reg_{name}_obj_1.png'
...
qc_file=f'reg_{name}_obj_2.png'
...
qc_file=f'reg_{name}_obj_3.png'
...
med_scale1=np.nanmedian(fib_scale1)
med_scale2=np.nanmedian(fib_scale2)
med_scale3=np.nanmedian(fib_scale3)
std_scale1=np.nanstd(fib_scale1)
std_scale2=np.nanstd(fib_scale2)
std_scale3=np.nanstd(fib_scale3)
...
for i in arange(0,331):
    if ((flux_ref[i]>5*f_e) and ((np.log10(fib_scale[i])<np.log10(med_scale*s)) or (np.log10(fib_scale[i])>np.log10(med_scale/s)))):
        if (((flux_ref[i+331]>15*f_e) and ((np.log10(fib_scale[i+331])<np.log10(med_scale*s)) or (np.log10(fib_scale[i+331])>np.log10(med_scale/s)))) or ((flux_ref[i+331]>15*f_e) and ((np.log10(fib_scale[i+662])<np.log10(med_scale*s)) or (np.log10(fib_scale[i+662])>np.log10(med_scale/s))))):
            print(i)
...
plt.savefig(f'{name}.broken_fib.png')
...
(crval1_cal,crval2_cal, crpix1_cal,crpix2_cal,e_crval1_cal, e_crval2_cal,rat_mean,rat_std,mag_PS, mag_PS_to_CAL,mag_CAL,FWHM_now,chi_now,rat_map)=get_astrometry(img_dec_r,filename_PS_r,ext=ext, xc_PS=df_s['XC1'][0],yc_PS=df_s['YC1'][0],RA_PS=df_s['CRVAL1_CR'][0] , DEC_PS=df_s['CRVAL2_CR'][0], xc_cal=df_s['XC'][0]/pix_size-radius , yc_cal=df_s['YC'][0]/pix_size-radius,band=band,i_range=i_range,label='org. recons.',FWHM_in=0.7,A_R=Ar_name)
